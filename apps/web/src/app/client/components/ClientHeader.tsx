...
export default function ClientHeader({ currentUserId, userName, userEmail, avatarUrl }: ClientHeaderProps) {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      console.error('[CLIENT HEADER] Failed to fetch notifications:', error)
    }
  }
  ...
  const handleNotificationClick = async (notification: Notification) => {
    ...
    router.push(`/client/orders/${notification.order_id}`)
  }
  ...
  const userInitial = userName?.[0] || 'C'
    ...
    <div className="flex items-center gap-4">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="text-xs">
          <p className="text-foreground font-medium">{userName || 'Client'}</p>
