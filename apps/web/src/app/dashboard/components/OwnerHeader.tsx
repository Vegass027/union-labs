...
export default function OwnerHeader({ currentUserId, userName, userEmail, avatarUrl }: OwnerHeaderProps) {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      console.error('[OWNER HEADER] Failed to fetch notifications:', error)
    }
  }
  ...
  const userInitial = userName?.[0] || 'O'
    ...
    <div className="flex items-center gap-4">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="text-xs">
          <p className="text-foreground font-medium">{userName || 'Owner'}</p>
